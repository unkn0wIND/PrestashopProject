...
class LeftBanner extends ObjectModel
{
	...
	public static $definition = array(
		'table' => 'ctleftbanner_slides',
		'primary' => 'id_ctleftbanner_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'ctleftbanner` (`id_shop`, `id_ctleftbanner_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'ctleftbanner`
			WHERE `id_ctleftbanner_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_ctleftbanner_slides` = hs.`id_ctleftbanner_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_ctleftbanner_slides` as id_slide
			FROM `'._DB_PREFIX_.'ctleftbanner_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'ctleftbanner` hs ON (hss.`id_ctleftbanner_slides` = hs.`id_ctleftbanner_slides`)
