...
include_once(_PS_MODULE_DIR_.'ct_leftbanner/LeftBanner.php');

class Ct_LeftBanner extends Module implements WidgetInterface
{
    protected $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('CT - Left Banner', array(), 'Modules.LeftBanner');
        ...
        $this->templateFile = 'module:ct_leftbanner/views/templates/hook/ctleftbanner.tpl';
    }
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'ctleftbanner` (
                `id_ctleftbanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
                `id_shop` int(10) unsigned NOT NULL,
                PRIMARY KEY (`id_ctleftbanner_slides`, `id_shop`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'ctleftbanner_slides` (
              `id_ctleftbanner_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
              `position` int(10) unsigned NOT NULL DEFAULT \'0\',
              `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
              PRIMARY KEY (`id_ctleftbanner_slides`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides lang configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'ctleftbanner_slides_lang` (
              `id_ctleftbanner_slides` int(10) unsigned NOT NULL,
              `id_lang` int(10) unsigned NOT NULL,
              `title` varchar(255) NOT NULL,
              `url` varchar(255) NOT NULL,
              `image` varchar(255) NOT NULL,
              PRIMARY KEY (`id_ctleftbanner_slides`,`id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');
    ...
    protected function deleteTables()
    {
        ...
        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'ctleftbanner`, `'._DB_PREFIX_.'ctleftbanner_slides`, `'._DB_PREFIX_.'ctleftbanner_slides_lang`;
        ');
    }
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute('
            INSERT IGNORE INTO '._DB_PREFIX_.'ctleftbanner (id_ctleftbanner_slides, id_shop)
            SELECT id_ctleftbanner_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'ctleftbanner
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
            ...
            FROM `'._DB_PREFIX_.'ctleftbanner_slides` hss, `'._DB_PREFIX_.'ctleftbanner` hs
            WHERE hss.`id_ctleftbanner_slides` = hs.`id_ctleftbanner_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
        );
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT hs.`id_ctleftbanner_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`image`
            FROM '._DB_PREFIX_.'ctleftbanner hs
            LEFT JOIN '._DB_PREFIX_.'ctleftbanner_slides hss ON (hs.id_ctleftbanner_slides = hss.id_ctleftbanner_slides)
            LEFT JOIN '._DB_PREFIX_.'ctleftbanner_slides_lang hssl ON (hss.id_ctleftbanner_slides = hssl.id_ctleftbanner_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'ct_leftbanner/views/img/'.$slide['image']);
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN '._DB_PREFIX_.'ctleftbanner_slides hss ON (hs.id_ctleftbanner_slides = hss.id_ctleftbanner_slides)
            LEFT JOIN '._DB_PREFIX_.'ctleftbanner_slides_lang hssl ON (hss.id_ctleftbanner_slides = hssl.id_ctleftbanner_slides)
            WHERE hs.`id_ctleftbanner_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_ctleftbanner_slides` as id_slide
                FROM `'._DB_PREFIX_.'ctleftbanner` hs
                WHERE hs.`id_ctleftbanner_slides` = '.(int)$id_slide;
