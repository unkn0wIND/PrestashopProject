...
include_once(_PS_MODULE_DIR_.'ct_imageslider/Ct_HomeSlide.php');

class Ct_ImageSlider extends Module implements WidgetInterface
{
    protected $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('CT - Image slider', array(), 'Modules.ImageSlider');
        $this->description = $this->getTranslator()->trans('Display Image Slider on Homepage.', array(), 'Modules.ImageSlider');
        $this->ps_versions_compliancy = array('min' => '1.7.0.0', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:ct_imageslider/views/templates/hook/slider.tpl';
    }
    ...
    public function install()
    {
        ...
        if (parent::install() &&
            ...
            foreach ($shops as $shop_id) {
                ...
                $res = Configuration::updateValue('CTHOMESLIDER_SPEED', $this->default_speed, false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('CTHOMESLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover, false, $shop_group_id, $shop_id);
            }

            /* Sets up Shop Group configuration */
            if (count($shop_groups_list)) {
                foreach ($shop_groups_list as $shop_group_id) {
                    $res &= Configuration::updateValue('CTHOMESLIDER_SPEED', $this->default_speed, false, $shop_group_id);
                    $res &= Configuration::updateValue('CTHOMESLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover, false, $shop_group_id);
                }
            }

            /* Sets up Global configuration */
            $res &= Configuration::updateValue('CTHOMESLIDER_SPEED', $this->default_speed);
            $res &= Configuration::updateValue('CTHOMESLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover);
    ...
    public function uninstall()
    {
        ...
        if (parent::uninstall()) {
            ...
            $res &= Configuration::deleteByName('CTHOMESLIDER_SPEED');
            $res &= Configuration::deleteByName('CTHOMESLIDER_PAUSE_ON_HOVER');
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'cthomeslider` (
                `id_cthomeslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
                `id_shop` int(10) unsigned NOT NULL,
                PRIMARY KEY (`id_cthomeslider_slides`, `id_shop`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'cthomeslider_slides` (
              `id_cthomeslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
              `position` int(10) unsigned NOT NULL DEFAULT \'0\',
              `active` tinyint(1) unsigned NOT NULL DEFAULT \'0\',
              PRIMARY KEY (`id_cthomeslider_slides`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');

        /* Slides lang configuration */
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'cthomeslider_slides_lang` (
              `id_cthomeslider_slides` int(10) unsigned NOT NULL,
              ...
              PRIMARY KEY (`id_cthomeslider_slides`,`id_lang`)
            ) ENGINE='._MYSQL_ENGINE_.' DEFAULT CHARSET=UTF8;
        ');
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new Ct_HomeSlide($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'cthomeslider`, `'._DB_PREFIX_.'cthomeslider_slides`, `'._DB_PREFIX_.'cthomeslider_slides_lang`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = Ct_HomeSlide::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function _postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            if (!Validate::isInt(Tools::getValue('CTHOMESLIDER_SPEED'))) {
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            foreach ($shops as $shop_id) {
                ...
                $res = Configuration::updateValue('CTHOMESLIDER_SPEED', (int)Tools::getValue('CTHOMESLIDER_SPEED'), false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('CTHOMESLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('CTHOMESLIDER_PAUSE_ON_HOVER'), false, $shop_group_id, $shop_id);
            }

            /* Update global shop context if needed*/
            switch ($shop_context) {
                case Shop::CONTEXT_ALL:
                    $res &= Configuration::updateValue('CTHOMESLIDER_SPEED', (int)Tools::getValue('CTHOMESLIDER_SPEED'));
                    $res &= Configuration::updateValue('CTHOMESLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('CTHOMESLIDER_PAUSE_ON_HOVER'));
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue('CTHOMESLIDER_SPEED', (int)Tools::getValue('CTHOMESLIDER_SPEED'), false, $shop_group_id);
                            $res &= Configuration::updateValue('CTHOMESLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('CTHOMESLIDER_PAUSE_ON_HOVER'), false, $shop_group_id);
                        }
                    }
                    break;
            ...
            $slide = new Ct_HomeSlide((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            $slide = new Ct_HomeSlide((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    {
        $this->context->controller->registerStylesheet('modules-cthomeslider', 'modules/'.$this->name.'/views/css/flexslider.css', ['media' => 'all', 'priority' => 150]);
        $this->context->controller->registerJavascript('modules-cthomeslider', 'modules/'.$this->name.'/views/js/jquery.flexslider-min.js', ['position' => 'bottom', 'priority' => 150]);
    }
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        ...
        return [
            'cthomeslider' => [
                'speed' => $config['CTHOMESLIDER_SPEED'],
                'pause' => $config['CTHOMESLIDER_PAUSE_ON_HOVER'] ? 'true' : 'false',
                'slides' => $slides,
            ],
        ];
    }
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute('
            INSERT IGNORE INTO '._DB_PREFIX_.'cthomeslider (id_cthomeslider_slides, id_shop)
            SELECT id_cthomeslider_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'cthomeslider
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
            ...
            FROM `'._DB_PREFIX_.'cthomeslider_slides` hss, `'._DB_PREFIX_.'cthomeslider` hs
            WHERE hss.`id_cthomeslider_slides` = hs.`id_cthomeslider_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
        );
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT hs.`id_cthomeslider_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`legend`, hssl.`description`, hssl.`image`
            FROM '._DB_PREFIX_.'cthomeslider hs
            LEFT JOIN '._DB_PREFIX_.'cthomeslider_slides hss ON (hs.id_cthomeslider_slides = hss.id_cthomeslider_slides)
            LEFT JOIN '._DB_PREFIX_.'cthomeslider_slides_lang hssl ON (hss.id_cthomeslider_slides = hssl.id_cthomeslider_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'ct_imageslider/views/img/'.$slide['image']);
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN '._DB_PREFIX_.'cthomeslider_slides hss ON (hs.id_cthomeslider_slides = hss.id_cthomeslider_slides)
            LEFT JOIN '._DB_PREFIX_.'cthomeslider_slides_lang hssl ON (hss.id_cthomeslider_slides = hssl.id_cthomeslider_slides)
            WHERE hs.`id_cthomeslider_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_cthomeslider_slides` as id_slide
                FROM `'._DB_PREFIX_.'cthomeslider` hs
                WHERE hs.`id_cthomeslider_slides` = '.(int)$id_slide;
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = Ct_HomeSlide::getAssociatedIdsShop((int)$slide['id_slide']);
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'name' => 'CTHOMESLIDER_PAUSE_ON_HOVER',
    ...
    public function getConfigFieldsValues()
    {
        ...
        return array(
            'CTHOMESLIDER_SPEED' => Tools::getValue('CTHOMESLIDER_SPEED', Configuration::get('CTHOMESLIDER_SPEED', null, $id_shop_group, $id_shop)),
            'CTHOMESLIDER_PAUSE_ON_HOVER' => Tools::getValue('CTHOMESLIDER_PAUSE_ON_HOVER', Configuration::get('CTHOMESLIDER_PAUSE_ON_HOVER', null, $id_shop_group, $id_shop)),
        );
    }
