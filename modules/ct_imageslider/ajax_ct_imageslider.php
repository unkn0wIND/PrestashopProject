...
include_once('ct_imageslider.php');

$home_slider = new Ct_ImageSlider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'cthomeslider_slides` SET `position` = '.(int)$position.'
			WHERE `id_cthomeslider_slides` = '.(int)$id_slide
		);
