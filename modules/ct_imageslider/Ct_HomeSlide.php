...
class Ct_HomeSlide extends ObjectModel
{
	public $title;
	public $description;
	public $url;
	...
	public static $definition = array(
		'table' => 'cthomeslider_slides',
		'primary' => 'id_cthomeslider_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'cthomeslider` (`id_shop`, `id_cthomeslider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'cthomeslider`
			WHERE `id_cthomeslider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_cthomeslider_slides` = hs.`id_cthomeslider_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_cthomeslider_slides` as id_slide
			FROM `'._DB_PREFIX_.'cthomeslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'cthomeslider` hs ON (hss.`id_cthomeslider_slides` = hs.`id_cthomeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new Ct_HomeSlide($row['id_slide']);
