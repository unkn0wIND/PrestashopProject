...
class Ct_Categorylist extends Module implements WidgetInterface
{
    protected $html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->trans('CT - Category List Block', array(), 'Modules.CategoryList');
        $this->description = $this->trans('Adds a block with sub category.', array(), 'Modules.CategoryList');
		
        $this->ps_versions_compliancy = array('min' => '1.7', 'max' => _PS_VERSION_);
		$this->templateFile = 'module:ct_categorylist/views/templates/front/ct_categorylist.tpl';
    }
	
	public function install()
    {	
		$arrayDefault = array('3','4','5','6');
        Configuration::updateValue("CT_CATE_LIST", serialize($arrayDefault));
	...
	public function installDB()
    {
        ...
        $return &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'ctcategorylist` (
			`id_ctcategorylist` int(11) NOT NULL AUTO_INCREMENT,
			`image` varchar(128) NOT NULL,
			`id_shop` int(10) NOT NULL,
			`name_category` varchar(128) NOT NULL,
			`id_category` int(10)  NOT NULL,		
			PRIMARY KEY (`id_ctcategorylist`)
    ...
    {
        Configuration::deleteByName('CT_CATE_LIST');
		
		$ret = true;
        if ($drop_table) {
            $ret &=  Db::getInstance()->execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'ctcategorylist`');
        }
    ...
    {
        $output = '';
        if (((bool)Tools::isSubmit('submitCtcategorylist')) == true) {
            $ids = Tools::getValue('CT_CATE_LIST');
            
			if(empty($ids) || !is_array($ids)) {
                $ids = array(1);
            }
            Configuration::updateValue('CT_CATE_LIST', serialize($ids));
        ...
        if (Tools::isSubmit('deletect_categorylist') && Tools::getValue('id_category')) {
    ...
    protected function renderForm()
    {
        ...
        $helper->submit_action = 'submitCtcategorylist';
    ...
    {
        $result =  array(
            'CT_CATE_LIST' =>  Tools::getValue('CT_CATE_LIST', Configuration::get('CT_CATE_LIST')),
            'category_id' => Tools::getValue('category_id', Configuration::get('category_id')),
        );

        if(!is_array($result['CT_CATE_LIST']) && !empty($result['CT_CATE_LIST'])) {
            $result['CT_CATE_LIST'] = unserialize($result['CT_CATE_LIST']);
        }
    ...
    {
        $result = Db::getInstance()->execute('INSERT  INTO `'._DB_PREFIX_.'ctcategorylist`(`id_category`,`name_category`,`image`,`id_shop`) VALUES ('.(int)$category_id.', \''.pSQL($name_cate).'\', \''.pSQL($filename).'\','.(int)$id_shop.')');
        return $result;
    }

    public function updatecategoryicon($category_id,$name_cate,$filename)
    {
        $result = Db::getInstance()->execute('UPDATE `'._DB_PREFIX_.'ctcategorylist` SET `name_category` = \''.pSQL($name_cate).'\',`image` =\''.pSQL($filename).'\' WHERE `id_category` = '.(int)$category_id);
        return $result;
    }

    public static function deleteCategoryId($category_id)
    {
        $sql = 'DELETE FROM`'._DB_PREFIX_.'ctcategorylist` WHERE `id_category` = '.(int)$category_id;
        Db::getInstance()->execute($sql);
    }
    
    public function getimage($category_id,$id_shop)
    {
        $sql = 'SELECT id_ctcategorylist ,image,id_category,name_category FROM '._DB_PREFIX_.'ctcategorylist WHERE id_category = '.(int)$category_id.' and id_shop ='.(int)$id_shop.'' ;
        return Db::getInstance()->executeS($sql);
    }

    public function getcategoryicon()
    {
        $sql = 'SELECT id_ctcategorylist ,image,id_category,name_category, id_shop FROM '._DB_PREFIX_.'ctcategorylist';
        return Db::getInstance()->executeS($sql);
    }
	
	public function hookdisplayHeader($params)
    {
        $this->context->controller->registerStylesheet('modules-ctcategorylist', 'modules/'.$this->name.'/views/css/ctcategorylist.css', ['media' => 'all', 'priority' => 150]);
    }
	
    public function renderWidget($hookName = null, array $configuration = [])
    {
        if (!$this->isCached($this->templateFile, $this->getCacheId('ct_categorylist'))) {
            $this->smarty->assign($this->getWidgetVariables($hookName, $configuration));
        }

        return $this->fetch($this->templateFile, $this->getCacheId('ct_categorylist'));
    }
	 ...
	 public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        ...
        $arrayCategory = array();
        $categoryimg = '';
        foreach ($config_values['CT_CATE_LIST'] as $id){
            if($id == 1) { continue; }
        ...
        return array(
             'ctcategoryinfos' => $arrayCategory,
			 'image_url' => $this->context->link->getMediaLink(_MODULE_DIR_.'ct_categorylist/views/img'),
        );
    }
}
