	...
	{if $start!=$stop}
		...
		{if $p != 1}
			...
			<li id="pagination_previous{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="pagination_previous">
				<a {$no_follow_text|escape:'html':'UTF-8'} href="{$link->goPage($requestPage, $p_previous)|escape:'html':'UTF-8'}" class="previous">
					<i class="icon-chevron-left"></i> <b>{l s='Previous' d='Modules.PsBlog.Shop'}</b></a>
			</li>
		{else}
			<li id="pagination_previous{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="previous disabled pagination_previous"><span><i class="icon-chevron-left"></i> <b>{l s='Previous' d='Modules.PsBlog.Shop'}</b></span></li>
		{/if}
		{if $start==3}
		...
		{if $pages_nb > 1 AND $p != $pages_nb}
			...
			<li id="pagination_next{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="pagination_next">	
				<a {$no_follow_text|escape:'html':'UTF-8'} href="{$link->goPage($requestPage, $p_next)|escape:'html':'UTF-8'}" class="next"><b>{l s='Next' d='Modules.PsBlog.Shop'}</b> <i class="icon-chevron-right"></i></a>
			</li>
		{else}
			<li id="pagination_next{if isset($paginationId)}_{$paginationId|escape:'html':'UTF-8'}{/if}" class="next disabled pagination_next"><span><b>{l s='Next' d='Modules.PsBlog.Shop'}</b> <i class="icon-chevron-right"></i></span></li>
		{/if}
		</ul>
	{/if}
	</div>
